...
export default function ProjectView({
  ...
  const [query, setQuery] = useState(
    links.backendLink("issues", `?page=${1}&project=${project.id}`)
  );
  ...
  if (res.response.errors)
    return (
      <Center height={"25vh"}>
        <ErrorMessage message={res.response.errors.error} />
      </Center>
    );
  ...
  return (
    <Box width={"full"} height={"full"} {...props}>
        ...
        {canEdit ? (
          <ButtonGroup size={"md"}>
            <DeleteAlertDialog
              ...
              message={
                "¿Estas seguro que quieres borrar el proyecto? Esta acción no se puede deshacer."
              }
              isDisabled
            />
            <DrawerAddIssue
...
const issueCard = (issue, reFetch, canEdit) => {
  ...
  return (
    <Flex key={issue.id} height={"auto"} width={"full"}>
      <Stack isInline>
        <Stack>
          <Stack isInline>
